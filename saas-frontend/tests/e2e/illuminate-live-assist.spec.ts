    ...
    try {
      ...
      for (let attempt = 0; attempt < 20; attempt++) {
        ...
        if (isVisible) {
          ...
          console.log(`[Live Assist] Customer visible in queue (attempt ${attempt + 1})`);
          break;
        }
        console.log(`[Live Assist] Waiting for customer in queue... (attempt ${attempt + 1}/20)`);
      }
      ...
      console.log('[Live Assist] Reading started — customer info visible');

      // Customer should see "in progress"
      await expect(customerPage.locator('[data-testid="live-in-progress"]')).toBeVisible({ timeout: 30000 });
      console.log('[Live Assist] Customer sees reading in progress');

      // ── Complete the reading (captures payment) ──
      ...
      console.log('[Live Assist] Reading completed — current reading card dismissed');

      // ── Send post-reading summary (must happen before reload destroys panel) ──
      const summaryPanel = practitionerPage.locator('[data-testid="summary-panel"]');
      ...
      console.log('[Live Assist] Post-reading summary sent');

      // Customer should see "completed" state with charge amount
      ...
      console.log('[Live Assist] Customer sees reading complete with charge');

      // Stats need time to update (Stripe capture + query invalidation) — poll with reload
      ...
      for (let attempt = 0; attempt < 10; attempt++) {
        ...
        if (completedText && /1/.test(completedText)) {
          statsUpdated = true;
          console.log(`[Live Assist] Stats updated (attempt ${attempt + 1})`);
          break;
        }
      ...
      await expect(practitionerPage.locator('[data-testid="stat-revenue"]')).toContainText('20', { timeout: 5000 });
      console.log('[Live Assist] Stats verified: 1 done, $20');
