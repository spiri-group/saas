...
export default function FulfillmentPage({ params }: { params: Promise<{ practitioner_slug: string; orderId: string }> }) {
  const { practitioner_slug, orderId } = use(params);
  const router = useRouter();

  // Get practitioner ID from slug
  const { data: practitionerData } = useQuery({
    queryKey: ['practitioner-from-slug', practitioner_slug],
    queryFn: async () => merchantIdFromSlug(practitioner_slug),
  });
  const practitionerId = practitionerData?.merchantId;
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const uploadResponse = await fetch('/api/azure_upload', {
        ...
        headers: {
          'container': 'public',
          'relative_path': `service-deliverables/${practitionerId}/${orderId}`
        },
        body: formData
      });
  ...
  const handleMarkDelivered = async () => {
    ...
    try {
      ...
      router.push(`/p/${practitioner_slug}/manage/services/orders`);
    } catch (error) {
  ...
  return (
    <div className="container mx-auto py-8 px-4 md:px-8">
      <Link
        href={`/p/${practitioner_slug}/manage/services/orders`}
