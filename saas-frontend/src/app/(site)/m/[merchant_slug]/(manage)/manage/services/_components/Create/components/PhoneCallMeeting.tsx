...
import { CheckCircle2Icon, PhoneCall } from "lucide-react";
import { escape_key } from "@/lib/functions";
import { useEffect } from "react";
import ComboBox from "@/components/ux/ComboBox";

export type phoneCallMeeting_type = z.infer<typeof phoneCallSchema>
...
type BLProps = {
    ...
    defaultConfiguration?: phoneCallMeeting_type,
    withIcons?: boolean
    onComplete: (value: phoneCallMeeting_type) => void
    onRemove: () => void
}
...
const useBL = (props: Props) => {
    ...
    const form = useForm<z.infer<typeof phoneCallSchema>>({
...
const PhoneCallMeeting : React.FC<Props> = ({withIcons=false, ...props}) => {
        ...
        <Popover>
            <PopoverTrigger asChild>
                <Button className="flex flex-row items-center space-x-3" variant={props.isSet ? "link" : "outline"}>
                    {!withIcons ? <>{props.isSet && <CheckCircle2Icon />}<span>Phone Call</span></> : <></> }
                    {withIcons && <PhoneCall size={15} />}
                </Button>
            </PopoverTrigger>
