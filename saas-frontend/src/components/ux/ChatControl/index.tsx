...
const ChatControl : React.FC<ChatControlProps> = ({ title = "Chat", withTitle = true, ...props}) => {
    ...
    const bodyText = isDark ? "text-white/90" : "text-base";
    const nameText = isDark ? "text-white font-bold text-sm" : "font-bold text-sm";
        ...
        <div className={cn("flex flex-col items-start h-full w-full min-h-0", props.className)}>
            {(bl.scopedMessage != null || withTitle) && (
                <div className="flex flex-row items-center -mt-2 w-full">
                    {withTitle && <h2 className={cn("text-lg font-bold", isDark && "text-white")}> {title} </h2>}
                </div>
            )}
            {bl.scopedMessage && (
                <div className={cn("w-full border-b-2 rounded-md p-4", isDark ? "border-white/30" : "border-primary")}>
                    <div className="flex flex-row items-center w-full">
                        <span className={cn("text-sm", mutedText)}>{format_time_string(bl.scopedMessage.sentAt)}</span>
                        <span className={cn("ml-2", nameText)}>{bl.scopedMessage.posted_by.name} </span>
                    ...
                    <div className="flex flex-row pt-2">
                        ...
                        <span className={cn("text-base text-wrap", isDark && "text-white/90")}>{bl.scopedMessage.text}</span>
                    </div>
                </div>
            )}
            {<span className={cn(bl.autoScrollEnabled ? (isDark ? "text-purple-300" : "text-primary") : mutedText, "text-sm mt-2 cursor-pointer")} onClick={() => bl.toggleAutoScroll()}>{bl.autoScrollEnabled ? "auto-scroll enabled" : "auto-scroll disabled"}</span>}
            {bl.isLoading && <span className={cn("text-sm mt-2", mutedText)}>Loading...</span>}
            {!bl.isLoading && bl.messages.data != null &&
                <>
                {
                    bl.messages.data.length > 0 ?
                    <ul ref={bl.chatControlRef} className={cn("flex flex-col overflow-y-auto divide-y flex-grow w-full", isDark && "divide-white/10", bl.scopedMessage != null ? "pl-6" : "")}>
                                        ...
                                        <div className="flex flex-col">
                                            <div className={classNames("flex flex-col")}>
                                                <div className="flex flex-row items-center">
                                                    <span className={cn("text-sm", mutedText)}>{format_time_string(message.sentAt)}</span>
                                                    <span className={cn("ml-2", nameText)}> {message.posted_by.name} </span>
                    ...
                    : <p className={cn("text-center text-base flex-grow", mutedText, bl.scopedMessage != null ? "pl-6 pt-3" : "pt-3 pl-2")}>Hmm.... No messages found. {props.readonly ? '' : `Let's get it started. Send your message!`}</p>
                }
                {!props.readonly &&
